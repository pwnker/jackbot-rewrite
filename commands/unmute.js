...
module.exports = {
    ...
    async execute(interaction) {
        ...
        if (!member.isCommunicationDisabled()) {
            return interaction.reply({content:`<@${member.id}> is not muted`,  ephemeral: true });
        }
    ...
    await interaction.reply({content:`Unmuted <@${user.id}>.`, ephemeral: true});
        ...
        dmEmbed = new MessageEmbed()
            .setColor("AQUA")
            .setTitle("Unmuted")
            .setDescription(`You have been unmuted in ${interaction.guild.name}. You can now interact with the server again.`)
            .setTimestamp()
