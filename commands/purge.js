...
module.exports = {
    ...
    async execute(interaction) {
        ...
        if (amount > 100 || amount < 1) {
            return interaction.reply({ content: 'You must enter a number between 1 and 100!', ephemeral: true });
        }
        ...
        if (user) {
            var messages = messages.filter(message => message.author.id === user.id);
        }
        ...
        confirmation = new MessageEmbed()
            ...
            .setFooter({text: `Purged by ${interaction.user.username} | ${interaction.user.id}`})
            .addFields(
                { name: 'Amount', value: `${amount}`, inline: true },
                { name: 'Username', value: user ? `<@${user.id}>`: `All users.`, inline: true },
                { name: 'Reason', value: reason, inline: true },
                { name: 'Channel', value: `<#${interaction.channel.id}>`, inline: true },
            )
            .setTimestamp()

        await interaction.client.channels.cache.get(process.env.LOG_CHANNEL).send({ embeds: [confirmation] });
    }
}