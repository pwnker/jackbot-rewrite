  ...
  data: new SlashCommandBuilder()
    ...
    .addStringOption((option) =>
      option
        .setName("color")
        .setDescription(
          'The color of the embed in HEX or name (e.g "Green"). Pass "INVIS" to create an embed with no color.'
        )
    )
    .addStringOption((option) =>
    ...
    .addAttachmentOption((option) =>
      option.setName("image").setDescription("The image to be embedded")
    )
    .addAttachmentOption((option) =>
      option
        .setName("small-image")
    ...
    .addStringOption((option) =>
      option
        .setName("url")
        .setDescription(
          "The url to go to when the title of the embed is clicked."
        )
    )
    .addAttachmentOption((option) =>
      option
  ...
  async execute(interaction) {
    ...
    if (interaction.options.getString("json")) {
      ...
      try {
        ...
        return await interaction.editReply({
          content:
            "Invalid embed! Please try again using a different structure.",
        });
      }
    } else {
      ...
      if (interaction.options.get("color")) {
        try {
          ...
          return await interaction.editReply({
            content:
              "Invalid color! Please try again using a number or name in capitals.",
          });
        }
      } else {
      ...
      if (interaction.options.get("fields")) {
        ...
        optionArray.forEach((e) => {
          ...
          jsonArray.push({
            name: arr[0],
            value: arr[1],
            inline: interaction.options.getBoolean("inline-fields")
              ? true
              : false,
          });
        });
        ...
        try {
          ...
          return await interaction.editReply({
            content:
              "Field parse error! Please try again using the format `name:value`.",
          });
        }
      }
      ...
      interaction.options.get("description")
        ...
        : null;
      interaction.options.get("footer")
      ...
      interaction.options.get("image")
        ...
        : null;
      interaction.options.get("small-image")
        ? embed.setThumbnail(
            interaction.options.getAttachment("small-image").url
          )
        : null;
