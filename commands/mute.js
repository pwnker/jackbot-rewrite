...
module.exports = {
    ...
    async execute(interaction) {
        ...
        if (user.roles && user.roles.cache.some(role => role.id === process.env.MOD_ROLE)) {
            return interaction.reply({content:'You cannot mute a moderator.',  ephemeral: true });
        }
    ...
    await member.timeout(time * multiplyer, reason).catch(err => {console.log(err)});
    await interaction.reply({content:`<@${user.id}> has been muted for ${duration}`,  ephemeral: true });
