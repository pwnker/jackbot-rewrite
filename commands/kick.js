...
module.exports = {
    ...
    async execute(interaction) {
        ...
        if (user.roles && user.roles.cache.some(role => role.id === process.env.MOD_ROLE )) {
            return interaction.reply({content:'You cannot kick a moderator.',  ephemeral: true });
        }
        ...
        .setColor("RED")
        .setTitle("Kicked")
        .setDescription(`You have been kicked from ${interaction.guild.name}. You can join back at any time however you should resolve the issue described in the kick reason to avoid further punishment.`)
        .setTimestamp()
    ...
    await interaction.reply({content:`<@${user.id}> has been kicked.`,  ephemeral: true });
        ...
        embed = new MessageEmbed()
            ...
            .setFooter({text:`Kicked by ${interaction.user.username} | ${interaction.user.id}`})
            .addFields(
